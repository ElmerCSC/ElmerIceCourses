...
! The variable Channel Area is defined on the edges only
          ...
          DO t=1,Solver % NumberOfActiveElements
            ...
            DO I=1,Element % TYPE % NumberOfNodes
              ...
              IF (MoulinMask(MoulinMaskPerm(k)) > 0.0) THEN
                ...
                !PRINT *,"HMM", MoulinFluxVal(MoulinFluxPerm(k)), t, k
              END IF
            END DO
        ...
        ELSE
           DO t=1, Mesh % NumberOfBoundaryElements
              ...
              BC => GetBC( Element )
              bc_id = GetBCId( Element )
              IF (bc_id == 13) PRINT *, "Moulin BC", bc_id, GetElementFamily()
              ...
              ! Moulin case (flux at node)
                ...
                IF ( ASSOCIATED( BC ) ) THEN            
                  ...
                  PRINT *,"Moulin Flux", MoulinFlux(1)
...
!!! TODO : do we need that as MASS = 0 here
                     ...
                     IF (Found) THEN
                       ...
                       END IF
                     END IF
                     !IF (Found) PRINT *, 'Moulin Flux', FORCE(1:N)
                     !IF (ANY( FORCE(1:N)  >  0.0_dp))  PRINT *, 'BC:', bc_id, ' Moulin Flux', FORCE(1:N)
                     IF (ASSOCIATED( QmSol )) THEN
                       DO i=1,N
                         j = Element % NodeIndexes(i)
                         QmSolution(QmPerm(Element % NodeIndexes(i))) = &
                            FORCE(i)-MASS(i,i)*(HydPot(HydPotPerm(j))-HydPotPrev(HydPotPerm(j),1))/dt
                       END DO
                     END IF
                     IF ( TransientSimulation ) THEN
                        CALL Default1stOrderTime( MASS, STIFF, FORCE, Element )
